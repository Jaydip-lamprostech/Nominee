...
contract Main is Ownable{
    ...
    mapping (address=>mapping(address=>address)) public ownerTokenIdToAddress;
    ...
    function addNomineesDetails(string memory name, string memory email, address nominee_address) public{
        addressToNominee[nominee_address]=Nominee(name,email,nominee_address,false);
        ownerToNominees[msg.sender].push(nominee_address);
    }

    ///@param name is the nominee's name, email is the nominee's email, and nominee_address
    ///is the nominee's address
    function editNomineeDetails(address _owner, address _oldNomineeAddress,string memory name, string memory email, address nominee_address) public{
        ...
        addressToNominee[_oldNomineeAddress].nominee_email = email;
            ...
            for (uint i=0; i<ownerToNominees[_owner].length;i++){
    ...
    function getNominees(address _owner) public view returns(address[] memory){
        return ownerToNominees[_owner];
    }

    /// @param _nominee is the nominee address
    /// @return nominee structure
    function getNomineeDetails(address _nominee) public view returns(Nominee memory){
    ...
    function assignAssetsToNominee(address _owner, address nominee_address,string memory _token_name,address _token_address,uint _approvedBalance,uint _nftId) public{
    ...
    //_nftId is the token id of nft.
    function ChangeAssetsToNomiee(address _owner, address old_nominee,address new_nominee,string memory _token_name,address _token_address,uint _approvedBalance,uint _nftId) public{
     ...
     function getAssetsToNominee(address _owner, address _token_address) public view returns(address){
         return ownerTokenIdToAddress[_owner][_token_address];
     }

     /// @return nominee address
     function checkIsNominated(address _owner, address _token_address) public view returns(bool){
    ...
    function setResponseDate(address _owner,string memory _date) public onlyOwner {
